...
def folha_pagamento(request):
    ...
    for item in funcionarios:
        ...
        relatorio.append({
            ...
            # 'inss': str(folha.inss if folha and folha.inss is not None else inss).replace('.', ','),

            'irrf': str(folha.irrf if folha else irrf).replace('.', ','),
            'desconto_vt': str(folha.desconto_vt if folha else desconto_vt).replace('.', ','),
            'salario_liquido': str(folha.salario_liquido if folha else salario_liquido).replace('.', ','),
            'pg_salario': folha.pg_salario if folha else '',
            'dias_extras': str(folha.dias_extras if folha else 0).replace('.', ','),
            'va': folha.va if folha else va,
            'vt': folha.vt if folha else vt,
            'pg_va_vt': folha.pg_va_vt if folha else '',
            'fgts': str(folha.fgts if folha else fgts).replace('.', ','),
            'obs': folha.obs if folha else '',
        })
