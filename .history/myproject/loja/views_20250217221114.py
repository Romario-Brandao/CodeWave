...
def servicos(request, cod_servico=None, cod_produto=None):
    ...
    if request.method == "POST":
        ...
        if action == "Lançar Serviço":
            ...
            pdf_path = "ordem_de_servico.pdf"

            # Criando o PDF
            c = canvas.Canvas(pdf_path, pagesize=A4)
            width, height = A4
            y = height - 30  # Ajuste a posição inicial do texto para diminuir a margem superior

            # Diminuir a fonte
            c.setFont("Helvetica", 12)  # Fonte menor (10 ao invés de 12 ou 14)
            c.drawString(40, y, "-------------------------- ORDEM DE SERVIÇO --------------------------")  # Ajuste a margem esquerda
            ...
            if tp > 0:
                c.setFont("Helvetica", 9)  # Ajuste da fonte para a tabela de produtos
                c.drawString(40, y, "------------------------- Produtos Vendidos -------------------------")
                y -= 15
