...
def dashboard(request):
    ...
    resumo_vendas = defaultdict(lambda: {"quantidade": 0, "total": 0, "ano": ""})

    for item in vendas:
        if item.vencimento:  # Garante que 'vencimento' não seja None
            mes_abrev = item.vencimento.strftime("%b")  # Obtém o mês abreviado em inglês
            mes = meses.get(mes_abrev, mes_abrev)  # Traduz para português
            ano = item.vencimento.year
            
            chave = f"{mes}/{ano}"  # Exemplo: "JAN/2024"
            
            resumo_vendas[chave]["quantidade"] += 1  # Contabiliza a venda
            resumo_vendas[chave]["total"] += item.total  # Soma o valor total da venda
