...
class OS(models.Model):
    ...
    valor_no_momento = models.DecimalField(max_digits=10, decimal_places=2, default=0)
...
class Folha_pagamento(models.Model):
    ...
    def __str__(self):
        return f"Compensação: {self.mes_ano_competencia} - Funcionario: {self.funcionario.nome} - Dias Extras: {self.dias_extras}"
...
class Custos_extras(models.Model):
    n_caixa = models.DecimalField(max_digits=100, decimal_places=0, default=0)
    referencia = models.DecimalField(max_digits=100, decimal_places=0, default=0)
    data_cadastro =  models.DateTimeField(default=datetime.now)
    parcela = models.CharField(max_length=10)
    forma_pg = models.CharField(max_length=10)
    fornecedor = models.ForeignKey(Fornecedor, null=True, blank=True, on_delete=models.SET_NULL)
    valor = models.DecimalField(max_digits=10, decimal_places=2, default=0)
    vencimento = models.DateField(null=False, blank=False)
    data_pg = models.DateField(null=True, blank=True)
    obs = models.CharField(max_length=50)
    usuario = models.CharField(max_length=20, null=True, blank=True, default="")
