    ...
    if request.method == 'POST':
        ...
        if acao == 'Lançar Custo':
            ...
            if Custos_extras.objects.filter(fornecedor=fornecedor, referencia=referencia, parcela=parcela, vencimento=vencimento).exists():
                erro = "Custo já cadastrados"
                print(erro)
            else:
                lancar = Custos_extras.objects.create(fornecedor=fornecedor,parcela=parcela, forma_pg=forma_pg, valor=valor,
                                                        referencia=referencia, vencimento=vencimento, data_pg=data_pg,obs=obs)
