...
@login_required(login_url='/')
def contas_receberrrr(request, id_titulo=None, data_inicial=None, data_final=None, filtro='todos', ordenar_por='os'):#
    relatorio = []
    dados_caixa = {}
    periodo = {}
    if request.method == "POST":
...
def contas_receber(request):
    ...
    #     valor = item.valor
    #     item.total = valor + acrescimo - desconto
    #     item.save()
    if filtro == "nao":
        vendas = vendas.filter(data_pg__isnull=True)
    elif filtro == "sim":
...
def dashboard(request):
    ...
    return render(request, 'dashboard.html', {'vendas':vendas, 'compras':compras, 'os_produtos':os_produtos})
