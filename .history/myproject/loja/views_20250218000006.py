    ...
    if request.method == "POST":
        ...
        if action == "Lançar Serviço":
            ...
            if tp > 0:
                ...
                c.drawString(40, y, f"{'Cod':<10}{'Produto':<45}{'Quantidade':>8}{'Valor Unitário':>18}{'Desconto:':>18}{'Total':>15}")
                y -= 15

                # Definindo as larguras fixas para cada coluna
                coluna_cod_produto = 40
                coluna_nome_produto = coluna_cod_produto + 40   # Largura para o código do produto
                coluna_quantidade = coluna_nome_produto + 45     # Largura para o nome do produto
                coluna_preco_unitario = coluna_quantidade + 15   # Largura para quantidade
                coluna_desconto = coluna_preco_unitario + 18     # Largura para o valor unitário
                coluna_total = coluna_desconto + 18              # Largura para desconto
                coluna_final = coluna_total + 15                 # Largura para total

                # Usando o espaço para formatar as colunas de maneira fixa
                for produto in produtos:
                    # Decrementa o valor de 'y' para a nova linha
                    y -= 8
                    
                    # Alinhando os valores nas colunas fixas
                    c.drawString(40, y, f"{produto.produto.cod_produto:<10}")
                    c.drawString(75, y, f"{produto.produto.nome:<45}")
                    c.drawString(120, y, f"{produto.quantidade:>8.3f}")
                    # c.drawString(coluna_preco_unitario, y, f"{produto.produto.preco_venda:>18.2f}")
