...
def folha_pagamento(request):
    ...
    if request.method == "POST":
        dados = request.POST.dict()
        if dados.get('acao') == 'Pagamento Salário':
            mes = dados.get('mes') 
            ano = dados.get('ano')
            mes_ano = f"{str(mes)}/{ano}"
    ...
    #     desconto_vt = salario * Decimal('0.06')
    #     fgts = str(round(salario * Decimal('0.07'), 2))
    #     salario_liquido = Decimal(salario - inss - desconto_vt - irrf)
    #     # -INSS - IRRF
    #     desconto_vt = str(round(desconto_vt, 2))
    #     inss = str(round(inss, 2))
    #     irrf = str(round(irrf, 2))
    #     salario_liquido = str(round(salario_liquido, 2))
    #     if Folha_pagamento.objects.filter(mes_ano_competencia=mes_ano, funcionario=item).exists():
    #         folha =Folha_pagamento.objects.get(mes_ano_competencia=mes_ano, funcionario=funcionario)            
    #         relatorio.append({
    #             'funcionario': item,
