...
def servicos(request, cod_servico=None, cod_produto=None):
    ...
    if request.method == "POST":
        ...
        if action == "Lançar Serviço":
            ...
            y -= 15
            c.drawString(40, y, f"Veículo: {veiculo.marca.nome} - {veiculo.marca.modelo}      Ano: {veiculo.ano}                  Placa: {veiculo.placa}")
            ...
            if produtos:
                y = height - 280  # Ajuste da posição do título para centralização
                c.setFont("Helvetica-Bold", 13)  # Fonte negrito e maior para destaque
                c.drawCentredString(width / 2, y, "Produtos Vendidos")  # Centralizando o texto
                y -= 25
                c.setFont("Helvetica-Bold", 10)
                
                # Definir as larguras fixas das colunas
                coluna_cod_produto = 40
                coluna_nome_produto = 75
                coluna_quantidade = 235
                coluna_valor_unitario = 285
                coluna_desconto = 360
                coluna_total = 420

                # Cabeçalho com larguras fixas
                c.drawString(coluna_cod_produto, y, f"{'Cod':<10}")
                c.drawString(coluna_nome_produto, y, f"{'Produto':<45}")
                c.drawString(coluna_quantidade, y, f"{'Quantidade':>8}")
                c.drawString(coluna_valor_unitario, y, f"{'Valor Unitário':>18}")
                c.drawString(coluna_desconto, y, f"{'Desconto':>18}")
                c.drawString(coluna_total, y, f"{'Total':>15}")
                
                y -= 15  # Desce para a próxima linha de dados
                c.setFont("Helvetica", 10)
                
                # Iterar sobre os produtos para preencher as colunas com os dados
                for produto in produtos:
                    # Decrementa o valor de 'y' para a nova linha
                    y -= 8
                    
                    # Alinhamento correto para cada coluna
                    ...
                    c.drawString(coluna_quantidade, y, f"{produto.quantidade:>8.3f}")
                    c.drawString(coluna_valor_unitario, y, f"{produto.produto.preco_venda:>18.2f}")
                    c.drawString(coluna_desconto, y, f"{produto.desconto:>18.2f}")
                    c.drawString(coluna_total, y, f"{produto.total:>18.2f}")
                    
                    # Decrementa 'y' para adicionar o espaçamento após o texto
                    y -= 4  # Ajuste para o espaçamento entre as linhas
                    
                    # Desenha uma linha horizontal abaixo do item
            ...
            if servicos:
                y = height - 280  # Ajuste da posição do título para centralização
                c.setFont("Helvetica-Bold", 13)  # Fonte negrito e maior para destaque
                c.drawCentredString(width / 2, y, "Serviços Realizados")  # Centralizando o texto
                y -= 25
                c.setFont("Helvetica-Bold", 10)
                ...
                # Cabeçalho com larguras fixas
                c.drawString(coluna_cod_produto, y, f"{'Cod':<10}")
                c.drawString(coluna_nome_produto, y, f"{'Serviço':<45}")
                c.drawString(coluna_valor_unitario, y, f"{'Valor':>18}")
                c.drawString(coluna_desconto, y, f"{'Desconto':>18}")
                c.drawString(coluna_total, y, f"{'Total':>15}")
                ...
                for servico in servicos:
                    ...
                    # Alinhamento correto para cada coluna
                    c.drawString(coluna_cod_produto, y, f"{servico.servico.cod_produto:<10}")
                    c.drawString(coluna_nome_produto, y, f"{servico.servico.nome:<45}")
                    c.drawString(coluna_quantidade, y, f"{servico.quantidade:>8.3f}")
                    c.drawString(coluna_valor_unitario, y, f"{servico.servico.preco_venda:>18.2f}")
                    c.drawString(coluna_desconto, y, f"{servico.desconto:>18.2f}")
                    c.drawString(coluna_total, y, f"{servico.total:>18.2f}")
            ...
            c.setFont("Helvetica", 9)  # Ajuste da fonte para a tabela de parcelas
            c.drawString(40, y, "------------------------ Parcelas da Venda ------------------------")
            y -= 15
            c.drawString(40, y, f"{'Parcela':<10}{'Forma de PG':<20}{'Valor':>12}{'Vencimento':>15}")
            y -= 15
            ...
            for i in range(1, quant_parcelas + 1):
                ...
                c.drawString(40, y, f"{parcela:<10}{forma_pg:<20}{valor:>12.2f}{vencimento_formatado:>15}")
                y -= 15
            ...
            if not pdf_path or not isinstance(pdf_path, str):
                raise ValueError("O caminho do PDF não foi definido corretamente.")

            pdf_abspath = o_s.path.abspath(pdf_path)


            # Criar comando para impressão
            # GHOSTSCRIPT_PATH = r"C:\Program Files\gs\gs10.00.0\bin\gswin64c.exe"  # Ajuste conforme necessário
            # GSPRINT_PATH = r"C:\Program Files\Ghostgum\gsview\gsprint.exe"  # Ajuste conforme necessário

            # if o_s.path.exists(GSPRINT_PATH):
            #     print_command = f'"{GSPRINT_PATH}" -printer "{printer_name}" "{pdf_abspath}"'
            #     o_s.system(print_command)
            # else:
