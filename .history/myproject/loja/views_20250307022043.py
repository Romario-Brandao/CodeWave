    ...
    if request.method == "POST":
        ...
        if action == "Lançar Serviço":
            ...
            if tp > 0:
                ...
                c.drawString(70, y, f"Produtos Vendidos OS: {os}")
                y -= 25
                ...
                for produto in produtos:
                    if y < 100:  # Se y for menor que 100, cria uma nova página
                        ...
                        c.setFillColorRGB(0.05, 0.05, 0.25)  # Azul Noite	
                        c.drawString(width - 180, 22, "CodeWave") 
                        c.setFont("Helvetica", 5)  # Fonte maior para o título
                        c.drawString(width - 180, 17, "___________") 
                        c.drawString(width - 147, 15, "dev") 
                        c.drawString(width - 135, 17, "___________") 
                        ...
                        c.setFont("Helvetica-Bold", 13)
                        c.drawString(70, y, f"Produtos Vendidos (Continuação OS: {os})")
                        y -= 25
                        c.setFont("Helvetica", 10)
                    y -= 8
            ...
            if ts > 0:
                ...
                c.drawString(70, y, f"Serviços Realizados OS: {os}")
                y -= 25
                c.setFont("Helvetica-Bold", 10)
                ...
                for servico in servicos:
                    if y < 100:  # Se não há mais espaço, nova página
                        ...
                        y = height - 50
                        c.setFont("Helvetica-Bold", 13)
                        c.drawString(70, y, f"Serviços Realizados (Continuação OS {os})")
                        y -= 25
                        c.setFont("Helvetica", 10)
            ...
            c.setFont("Helvetica-Bold", 13)
            c.drawString(70, y, f"Parcelas da Venda / Serviços OS: {os}")
            y -= 25
            ...
            if observacoes:
                ...
                for obs in observacoes:
                    if y < 100:  # Se não há mais espaço, nova página
                        ...
                        # c.setFillColorRGB(0.0, 0.2, 0.3)  # Azul Petróleo	
                        # c.setFillColorRGB(0.0, 0.0, 0.3)  # Azul Marinho Escuro	
                        # c.setFillColorRGB(0.0, 0.2, 0.6)  # Azul Cobalto	
                        c.setFillColorRGB(0.05, 0.05, 0.25)  # Azul Noite	
                        c.drawString(width - 180, 22, "CodeWave") 
                        c.setFont("Helvetica", 5)  # Fonte maior para o título
                        c.drawString(width - 180, 17, "___________") 
                    ...
                    for linha in linhas:
                        if y < 100:  # Se não há mais espaço, nova página
                            ...
                            c.setFillColorRGB(0.05, 0.05, 0.25)  # Azul Noite	
                            c.drawString(width - 180, 22, "CodeWave") 
                            c.setFont("He  lvetica", 5)  # Fonte maior para o título
                            c.drawString(width - 180, 17, "___________") 
                            c.drawString(width - 147, 15, "dev") 
                            c.drawString(width - 135, 17, "___________") 
                            c.setFont("Helvetica-Bold", 9)  # Fonte maior para o título
            ...
            if y < 105:  # Se não há mais espaço, nova página
                c.setFont("Helvetica-Bold", 15)
                # c.setFillColorRGB(0, 0, 0.4)  #
                # c.setFillColorRGB(0.0, 0.2, 0.3)  # Azul Petróleo	
                # c.setFillColorRGB(0.0, 0.0, 0.3)  # Azul Marinho Escuro	
                ...
                y = height - 50
                c.setFont("Helvetica-Bold", 13)
                c.drawString(70, y, f"Recebimento (Continuação OS {os})")
                y -= 25
...
@login_required(login_url='/')
def outras_despesas(request):
    nome_usuario = request.user.username
    dados = request.POST.dict()
    filtro = 'nao'
    ordenar_por = 'vencimento'
    fornecedores = Fornecedor.objects.order_by('nome')
    max_os = Custos_extras.objects.aggregate(Max('n_servico'))['n_servico__max']
    n_servico = (max_os or 0) + 1

    if request.method == "POST":
        n_servico = dados.get('n_servico') or n_servico
        filtro = dados.get('titulos_pg', 'nao')
        ordenar_por = dados.get('ordenar_por', 'vencimento')

    quant_parcelas = 0
    quant_parcelas2 = []
    fornecedor = fornecedores.filter(id=int(dados.get('fornecedor'))).first() if dados.get('fornecedor') and dados.get('fornecedor').isdigit() else None
    referencia = dados.get('referencia') or ''
    forma_pg = dados.get('forma_pg') or ''
    valor = dados.get('valor') or ''
    if request.method == 'POST':
        acao = dados.get('acao')
        if acao == 'Lançar Custo':
            quant_parcelas = int(dados.get('quant_parcelas'))
            for i in range(1, quant_parcelas + 1):
                parcela = dados.get(f'parcela_{i}', '')
                valor = dados.get(f'valor_{i}', '')
                vencimento = dados.get(f'vencimento_{i}', '')
                lancar = Custos_extras.objects.create(
                                                    n_servico=n_servico,
                                                    ...
                                                    vencimento=vencimento,  # Passando diretamente como string 'YYYY-MM-DD'
                                                )
            n_servico = int(n_servico) + 1
            fornecedor = ''
            referencia = ''
            forma_pg = ''
            valor = ''
            quant_parcelas = 0
            quant_parcelas2 = []
        if acao == 'Lançar Pagamento':
            data_pg = dados.get('pagamento')
            for chave, valor in dados.items():
                custos = Custos_extras.objects.all()
                if chave.startswith('custo'):
                    custo = custos.get(id=valor)
                    desconto = Decimal(dados.get(f'desconto{valor}', 0))
                    acrescimo = Decimal(dados.get(f'acrescimo{valor}', 0))
                    obs = dados.get(f'obs{valor}')
                    total = custo.valor + acrescimo - desconto
                    custo.desconto = desconto
                    custo.acrescimo = acrescimo
                    custo.total = total
                    custo.data_pg = data_pg
                    custo.obs = obs
                    custo.save()
        else:
            quant_parcelas = dados.get('quant_parcelas')
    ...
    custos = Custos_extras.objects.all()
    if filtro == "nao":
        custos = custos.filter(data_pg__isnull=True)
    elif filtro == "sim":
        custos = custos.filter(data_pg__isnull=False)
    custos = custos.order_by(ordenar_por)
    
    total_custo = custos.aggregate(total=Sum('total'))['total'] or 0
    quant_custos = custos.count()
    desconto_geral = custos.aggregate(desconto=Sum('desconto'))['desconto'] or 0
    acrescimo_geral = custos.aggregate(acrescimo=Sum('acrescimo'))['acrescimo'] or 0


    context = {'filtro':filtro,'ordenar_por':ordenar_por, 'nome_usuario':nome_usuario,
                'total_custo':total_custo,'quant_custos':quant_custos,'desconto_geral':desconto_geral,'acrescimo_geral':acrescimo_geral,
                'fornecedores':fornecedores, 'fornecedor':fornecedor, 'valor':valor, 'forma_pg': forma_pg, 'referencia': referencia,
               'n_servico':n_servico, 'quant_parcelas':quant_parcelas, 'quant_parcelas2':quant_parcelas2, 'custos':custos}
    return render(request, 'outras_despesas.html', context)
...
def contas_receber(request):
    ...
    if request.method == 'POST':
        ...
        if acao == 'Lançar Pagamento':
            data_pg = dados.get('pagamento')
            for chave, valor in dados.items():
                if chave.startswith('venda'):
                    venda = vendas.get(id=valor)
                    desconto = Decimal(dados.get(f'desconto{valor}', 0))
                    acrescimo = Decimal(dados.get(f'acrescimo{valor}', 0))
                    obs = dados.get(f'obs{valor}')
                    total = venda.valor + acrescimo - desconto
                    venda.desconto = desconto
                    venda.acrescimo = acrescimo
                    venda.total = total
                    venda.data_pg = data_pg
                    venda.obs = obs
                    venda.save()
    ...
    total_venda = vendas.aggregate(total=Sum('total'))['total'] or 0
    quant_venda = vendas.count()
    ...
    context = {'filtro':filtro,'ordenar_por':ordenar_por, 'nome_usuario':nome_usuario,
                'total_venda':total_venda,'quant_venda':quant_venda,'desconto_geral':desconto_geral,'acrescimo_geral':acrescimo_geral,
                'vendas':vendas}
    # return render(request, 'outras_despesas.html', context)
...
def contas_pagar(request):
    ...
    if request.method == 'POST':
        ...
        if acao == 'Lançar Pagamento':
            ...
            for chave, valor in dados.items():
                ...
                if chave.startswith('compra'):
                    compra = compras.get(id=valor)
                    desconto = Decimal(dados.get(f'desconto{valor}', 0))
                    acrescimo = Decimal(dados.get(f'acrescimo{valor}', 0))
                    obs = dados.get(f'obs{valor}')
                    total = compra.valor + acrescimo - desconto
                    compra.desconto = desconto
                    compra.acrescimo = acrescimo
                    compra.total = total
                    compra.data_pg = data_pg
                    compra.obs = obs
                    compra.save()
    ...
    total_compra = compras.aggregate(total=Sum('total'))['total'] or 0
    quant_compra = compras.count()
    ...
    context = {'filtro':filtro,'ordenar_por':ordenar_por, 'nome_usuario':nome_usuario,
                'total_compra':total_compra,'quant_compra':quant_compra,'desconto_geral':desconto_geral,'acrescimo_geral':acrescimo_geral,
                'compras':compras}
...
def calcular_descontos(salario):
    ...
    fgts = round(salario * Decimal('0.07'), 2)
    salario_liquido = salario - inss - desconto_vt - irrf

    return str(round(inss, 2)), str(round(irrf, 2)), str(round(desconto_vt, 2)), str(round(fgts, 2)), str(round(salario_liquido, 2))
...
def folha_pagamento(request):
    ...
    quant_dias = Dias_uteis.objects.get(mes_ano=mes_ano) if Dias_uteis.objects.filter(mes_ano=mes_ano).exists() else 0
    va = Decimal(20) * Decimal(quant_dias.quant_dias) if quant_dias != 0 else 0 # V.A = 20 reais por dia
    vt = 0
    relatorio = []
    ...
    if request.method == "POST":
        ...
        if dados.get('acao') == 'Gravar':
            ...
            if Dias_uteis.objects.filter(mes_ano=mes_ano).exists():
                ...
                for chave, valor in dados.items():
                    if chave.startswith('funcionario'):
                        ...
                        obs_key = f'obs{valor}'  # Construir a chave
                        obs = dados.get(obs_key, 0)  # Recuperar o valor de 'dias_extras'
            ...
            ano = dados.get('ano')
            mes_ano = f"{str(mes)}/{ano}"
            pagamento_salario = dados.get('pagamento_salario', '')
            for chave, valor in dados.items():
                if chave.startswith('funcionario'):
                    funcionario = funcionarios.get(id=valor)
                    salario = Decimal(funcionario.salario)
                    inss, irrf, desconto_vt, fgts, salario_liquido = calcular_descontos(salario)

                    if quant_dias != 0:
                        vt = Decimal(funcionario.vt_diario) * Decimal(quant_dias.quant_dias)

                    obs_key = f'obs{valor}'  # Construir a chave
                    obs = dados.get(obs_key, 0)  # Recuperar o valor de 'OBS'
                    ...
                    if not created:
                        folha.salario_bruto = funcionario.salario
                        folha.inss = inss
                        folha.irrf = irrf
                        folha.desconto_vt = desconto_vt
                        folha.salario_liquido = salario_liquido
                        folha.pg_salario = pagamento_salario
                        folha.fgts = fgts
                        folha.obs = obs
                        folha.save()
    ...
    for item in funcionarios:
        ...
        print(irrf)
        if quant_dias != 0:
            vt = Decimal(item.vt_diario) * Decimal(quant_dias.quant_dias)

        if Folha_pagamento.objects.filter(mes_ano_competencia=mes_ano, funcionario=item).exists():
        ...
        relatorio.append({
            'funcionario': item,
            'inss': str(folha.inss if folha and folha.inss else inss).replace('.', ','),
            'irrf': str(folha.irrf if folha and folha.irrf else irrf).replace('.', ','),
            'desconto_vt': str(folha.desconto_vt if folha and folha.desconto_vt else desconto_vt).replace('.', ','),
            'salario_liquido': str(folha.salario_liquido if folha and folha.salario_liquido else salario_liquido).replace('.', ','),
            'pg_salario': folha.pg_salario if folha and folha.pg_salario else '',
            'dias_extras': str(folha.dias_extras if folha and folha.dias_extras else 0).replace('.', ','),
            'va': folha.va if folha and folha.va else va,
            'vt': folha.vt if folha and folha.vt else vt,
            'pg_va_vt': folha.pg_va_vt if folha and folha.pg_va_vt else '',
            'fgts': str(folha.fgts if folha and folha.fgts else fgts).replace('.', ','),
            'obs': folha.obs if folha and folha.obs else '',
        })
    ...
    context = {
        ...
        'mes_atual':mes_atual,
        'ano_atual':ano_atual,
        'quant_dias':quant_dias.quant_dias if quant_dias else '',
...
def dashboard(request):
    ...
    vendas = Contas_receber.objects.all()
    os_produtos = OS_Produto.objects.all()
    compras = Contas_a_pagar.objects.all()
    ...
    for item in folha:
        ...
        vt = item.vt
        fgts = item.fgts
        salario = item.salario_bruto
        desconto_vt = item.desconto_vt
        valor_total += va + vt + fgts + salario - desconto_vt
    # print(f'Valor total da folha é R$ {valor_total}')


    custos_ex = Custos_extras.objects.all()

    # Resumo de custos gerais
    meses = {
    ...
    # Resumo de vendas (agrupando por mês e ano)
    resumo_vendas = defaultdict(lambda: {"quantidade": 0, "total": 0, "mes": "", "ano": ""})
    
    for item in vendas:
        if item.vencimento:
            mes_abrev = item.vencimento.strftime("%b")
            mes = meses.get(mes_abrev, mes_abrev)
            ano = item.vencimento.year
            
            chave = f"{mes}/{ano}"
            
            resumo_vendas[chave]["total"] += item.total
            resumo_vendas[chave]["mes"] = mes
            resumo_vendas[chave]["ano"] = ano

    resumo_vendas = [{"mes": v["mes"], "ano": v["ano"], "quantidade": v["quantidade"], "total": v["total"]} for v in resumo_vendas.values()]

    # Resumo de custos gerais (ajustado para considerar Folha_pagamento corretamente)
    resumo_custos = defaultdict(lambda: {"total": 0, "mes": "", "ano": "", "tipo_custo": ""})

    # Agrupar custos de compras e custos_extras por vencimento
    for item in list(compras) + list(custos_ex):
        if item.vencimento:
            mes_abrev = item.vencimento.strftime("%b")
            mes = meses.get(mes_abrev, mes_abrev)
            ano = item.vencimento.year
            ...
            # Adicionando o tipo de custo
            if item in compras:
                tipo_custo = "Compra"
            else:
                tipo_custo = "Custo Extra"
                
            resumo_custos[chave]["total"] += item.total
            resumo_custos[chave]["mes"] = mes
            resumo_custos[chave]["ano"] = ano
            resumo_custos[chave]["tipo_custo"] = tipo_custo

    # Agrupar folha de pagamento pelo campo `mes_ano_competencia`
    for item in folha:
        if item.mes_ano_competencia:
            partes = item.mes_ano_competencia.split("/")
            
            if len(partes) == 2:  # Garantimos que há mês e ano
                mes_num = partes[0].zfill(2)  # Garantir que o mês tem 2 dígitos
                ano = partes[1]
                mes = list(meses.values())[int(mes_num) - 1]  # Convertendo mês numérico para o nome abreviado
                
                chave = f"{mes}/{ano}"
                vt = item.vt or 0
                va = item.va or 0
                fgts = item.fgts or 0
                salario = item.salario_bruto or 0
                desconto_vt = item.desconto_vt or 0
                # print(f"Folha {mes}/{ano}: Salário Bruto: {salario}, VT: {vt}, VA: {va}, FGTS: {fgts}, Desconto VT: {desconto_vt}")

                # Verifique se a chave já existe no resumo_custos
                if chave not in resumo_custos:
                    resumo_custos[chave] = {"total": 0}

                # Calcula o total para o tipo de custo
                resumo_custos[chave]["total"] += salario_bruto + vt + va + fgts - desconto_vt
                
                # Aqui você coloca o print para depuração
                print(f"Chave: {chave}, Total Calculado: {resumo_custos[chave]['total']}")


                # Verificação de valores

                if not item.pg_va_vt:  # Se não foi pago, zera VT e VA
                    vt = 0
                    va = 0

                if not item.pg_salario:  # Se não foi pago, zera FGTS
                    fgts = 0

                resumo_custos[chave]["total"] += salario + vt + va + fgts - desconto_vt
                resumo_custos[chave]["mes"] = mes
                resumo_custos[chave]["ano"] = ano
                resumo_custos[chave]["tipo_custo"] = "Folha"

                # print(f"Resumo de custos: {resumo_custos.total}")  # Verificação do cálculo final


    resumo_custos = [{"mes": v["mes"], "ano": v["ano"], "total": v["total"], "tipo_custo": v["tipo_custo"]} for v in resumo_custos.values()]
