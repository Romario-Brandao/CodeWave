...
def servicos(request, cod_servico=None, cod_produto=None):
    ...
    if request.method == "POST":
        ...
        if action == "Lançar Serviço":
            ...
            servico = OS.objects.create(os=os, servico=servico, valor_no_momento=valor_servico, acrescimo=acrescimo, desconto=desconto, veiculo=veiculo, 
...
def os_por_cliente(request, cnpj):
    ...
    # Buscar todas as OS associadas ao cliente via Contas_receber
    os_ids = Contas_receber.objects.filter(pagador=cliente).values_list('os', flat=True)
    
    # Filtrar as OS usando os IDs encontrados
    os = OS.objects.select_related("servico").all()
    for item in os:
        item.valor_no_momento = item.servico.valor if item.servico else 0  # Define 0 se não houver serviço
    OS.objects.bulk_update(os, ["valor_no_momento"])

    os_produtos = OS_Produto.objects.select_related("produto").all()
    for item in os_produtos:
        item.valor_no_momento = item.produto.preco_venda if item.produto else 0  # Define 0 se não houver produto
    OS_Produto.objects.bulk_update(os_produtos, ["valor_no_momento"])
    ...
    # Filtrar os produtos dessas OSs
    os_produtos = OS_Produto.objects.filter(os__in=os_ids)


    context = {'os': os, 'os_produtos': os_produtos}
