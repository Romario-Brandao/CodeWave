...
def servicos(request, cod_servico=None, cod_produto=None):
    ...
    # Debug: Exibir os valores no console para verificar se estão chegando
    print("Código do Produto:", cod_produto)
    ...
    if request.method == "POST":
        ...
        if action == "Lançar Serviço":
            ...
            servico = OS.objects.create(os=os, servico=servico, acrescimo=acrescimo, desconto=desconto, veiculo=veiculo, 
            ...
            produto.save()
            venda_produto = OS_Produto.objects.create(os=os, produto=produto, quantidade=quantidade, desconto=desconto, total=total)
            cod_produto = ""
            n_servico = os
...
def folha_pagamento(request):
    ...
    for item in funcionarios:
        ...
        relatorio.append({
            'funcionario': item,
            'inss': folha.inss if folha.inss is not None else inss,
            'irrf': folha.irrf if folha.irrf is not None else irrf,
            'desconto_vt': folha.desconto_vt if folha.desconto_vt is not None else desconto_vt,
            'salario_liquido': folha.salario_liquido if folha.salario_liquido is not None else salario_liquido,
            'dias_extras': folha.dias_extras if folha.dias_extras is not None else 0,
            'va': folha.va if folha.va is not None else va,
            'vt': folha.vt if folha.vt is not None else vt,
            'pg_va_vt': folha.pg_va_vt if folha.pg_va_vt is not None else '',
            'fgts': folha.fgts if folha.fgts is not None else fgts,
            'obs': folha.obs if folha.obs is not None else '',
})
        ...
        if dados.get('acao') == 'Gravar':
            ...
            for chave, valor in dados.items():
                if chave.startswith('funcionario'):
                    ...
                    vt = Decimal(funcionario.vt_diario * dias_uteis.quant_dias)
                    va = Decimal(20 * dias_uteis.quant_dias)
