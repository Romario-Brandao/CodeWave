...
def outras_despesas(request):
    ...
    if request.method == 'POST':
        ...
        if acao == 'Lançar Custo':
            ...
            if Custos_extras.objects.filter(fornecedor=fornecedor, referencia=referencia, parcela=parcela).exists():#vencimento=vencimento
                ...
                lancar = Custos_extras.objects.create(fornecedor=fornecedor,parcela=parcela, forma_pg=forma_pg, valor=valor,
                                                        referencia=referencia, data_pg=data_pg,obs=obs)#vencimento=vencimento
