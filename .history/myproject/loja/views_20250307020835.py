...
def dashboard(request):
    ...
    for item in folha:
        print(f"Processando Folha de Pagamento: {item.mes_ano_competencia}")  # Depuração para ver os dados da folha
        
        if item.mes_ano_competencia:
            partes = item.mes_ano_competencia.split("/")
            
            if len(partes) == 2:  # Garantimos que há mês e ano
                mes_num = partes[0]  # Exemplo: "2" para o mês
                ano = partes[1]  # Exemplo: "2025"
                
                # Ajustando o mês para ter sempre dois dígitos
                mes_num = mes_num.zfill(2)  # Garante que o mês tenha dois dígitos, como "02"
                ...
                if not item.pg_va_vt:  # Se ainda não foi pago, zera VT e VA
                    vt = 0
                    va = 0

                if not item.pg_salario:  # Se ainda não foi pago, zera FGTS
                    fgts = 0

                print(f"Chave do resumo_custos: {chave}, Salário Bruto: {item.salario_bruto}, VT: {vt}, VA: {va}, FGTS: {fgts}")  # Depuração para verificar os cálculos

                resumo_custos[chave]["total"] += item.salario_bruto + vt + va + fgts - desconto_vt
                resumo_custos[chave]["mes"] = mes
                resumo_custos[chave]["ano"] = ano
    ...
    print(resumo_custos)  # Depuração final para ver o resumo de custos
