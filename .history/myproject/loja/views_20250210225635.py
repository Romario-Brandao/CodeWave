...
def os_por_cliente(request, cnpj):
    nome_usuario = request.user.name
...
def servicos_por_veiculo(request, placa):
    nome_usuario = request.user.name
    veiculo = Veiculos.objects.get(placa=placa)
    os_ids = contas_pagar.objects.filter(placa=placa).values_list('os', flat=True)
    os = Contas_receber.objects.filter(os__in=os_ids)
    return render(request, 'servicos_por_veiculo.html', {"os", flat=True})
########################################### Dasboard ###########################################
