...
def outras_despesas(request):
    dados = request.POST.dict()
    filtro = 'todos'
    ordenar_por = 'fornecedor'
    fornecedores = Fornecedor.objects.order_by('nome')
    if request.method == "POST":
        n_servico = dados.get('n_servico')
        filtro = dados.get('titulos_pg')
    ...
    quant_parcelas = 1
    quant_parcelas2 = [1]
    fornecedor = fornecedores.get(id=dados.get('fornecedor')) if fornecedores.filter(id=dados.get('fornecedor')) else ''
    ...
    if request.method == 'POST':
        ...
        if acao == 'Lançar Custo':
            ...
            vencimento = dados.get('vencimento') or ''  # já no formato 'YYYY-MM-DD'
            data_pg = dados.get('data_pg') or ''
            obs = dados.get('obs') or ''

            # Não é necessário converter, pois é uma data
            if Custos_extras.objects.filter(fornecedor=fornecedor, referencia=referencia, parcela=parcela).exists():
                erro = "Custo já cadastrado"
            else:
    ...
    total_custo = total_custo + acrescimo_geral - desconto_geral
