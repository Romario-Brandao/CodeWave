...
def outras_despesas(request):
    ...
    referencia = dados.get('referencia') if dados.get('referencia') else ''
    forma_pg = dados.get('forma_pg') if dados.get('forma_pg') else ''
    ...
    if request.method == 'POST':
        ...
        if acao == 'Lançar Custo':
            ...
            print(f"Vencimento recebido: {vencimento}")  # Debug para verificar o formato da data
    ...
    context = {'fornecedores':fornecedores, 'fornecedor':fornecedor, 'valor':valor, 'forma_pg': forma_pg, 'referencia': referencia,
               'n_servico':n_servico, 'quant_parcelas':quant_parcelas, 'quant_parcelas2':quant_parcelas2}
