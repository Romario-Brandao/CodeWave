    ...
    if request.method == "POST":
        ...
        if action == "Lançar Serviço":
            ...
            for i in range(1, quant_parcelas + 1):
                ...
                lancar = Contas_receber.objects.create(os=numero_os, parcela=parcela, forma_pg=forma_pg, pagador=pagador,
            ...
            if ts > 0:
                servicos = OS.objects.filter(os=numero_os)
                for servico in servicos:
                    servico.status = True
                    servico.veiculo = Veiculos.objects.get(id=veiculo)
                    servico.save()
            elif ts < 1:
                servico = OS.objects.create(os=numero_os, status=True, obs="Somente venda de produtos")
            if tp > 0:
                produtos = OS_Produto.objects.filter(os=numero_os)
            ...
            pdf_path = os.path.join(BASE_DIR, "static", "imagens", "os", f"os_{numero_os}.pdf")
            ...
            y -= 20  # Diminuir o espaçamento entre as linhas
            ...
            # Desenhar a linha completa logo abaixo do título
            y -= 10  # Ajuste da posição da linha
            ...
            if tp > 0:
                ...
                y -= 15
                c.drawString(40, y, f"{'Cod':<6}{'Produto':<30}{'Qtd':>8}{'Valor Unitário':>18}{'Total':>15}")
                y -= 15
                c.drawString(40, y, '-' * 85)
                y -= 15

                for produto in produtos:
                    c.drawString(40, y, f"{produto.produto.cod_produto:<6}{produto.produto.nome:<30}{produto.quantidade:>8.3f}{produto.produto.preco_venda:>18.2f}{produto.total:>15.2f}")
                    y -= 15
