...
def servicos(request, cod_servico=None, cod_produto=None):
    ...
    if request.method == "POST":
        ...
        if action == "Lançar Serviço":
            ...
            # c.setFillColorRGB(0.0, 0.2, 0.3)  # Define a cor azul
            # c.drawString(width - 180, 22, "CodeWave")  # Escrever o título no canto esquerdo
            # c.setFont("Helvetica", 5)  # Fonte maior para o título
            # c.drawString(width - 180, 17, "___________")  # Escrever o título no canto esquerdo
            # c.drawString(width - 147, 15, "DEV")  # Escrever o título no canto esquerdo
            # c.drawString(width - 135, 17, "___________")  # Escrever o título no canto esquerdo
            ...
            c.setStrokeColorRGB(0.0, 0.2, 0.3)  # Azul escuro para o contorno
            c.setFillColor(white)  # Sem preenchimento sólido
            c.drawString(width - 180, 22, "CodeWave")  # Contorno do texto

            # Criar o efeito pontilhado dentro das letras
            random.seed(42)
            for _ in range(150):  # Número de pontos dentro do texto
                x = random.uniform(width - 180, width - 100)  # Ajuste a área conforme necessário
                y = random.uniform(12, 26)
                c.setFillColor(blue)
                c.circle(x, y, 0.4, fill=1, stroke=0)  # Pequenos círculos para o efeito pontilhado

            # Continuar com o restante do seu código normalmente
            c.setFont("Helvetica", 5)
            ...
            # Definir fonte menor para os contatos
            c.setFont("Helvetica", 5)
            c.drawRightString(width - 62, 25, "71 9 1111-1111")
            c.drawRightString(width - 62, 20, "71 9 1111-1112")
            c.drawRightString(width - 40, 15, "codewave.rb@gmail.com")
