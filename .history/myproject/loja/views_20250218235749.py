    ...
    if request.method == "POST":
        ...
        if action == "Lançar Serviço":
            ...
            if produtos:
                ...
                y -= 25
            if servicos:
                y = height - 220  # Ajuste da posição do título para centralização
                c.setFont("Helvetica-Bold", 13)  # Fonte negrito e maior para destaque
                ...
                for servico in servicos:
                    ...
                    # Alinhamento correto para cada coluna
                    c.drawString(coluna_cod_produto, y, f"{servico.servico.id:<10}")
                    c.drawString(coluna_nome_produto, y, f"{servico.servico.nome:<45}")
                    c.drawString(coluna_quantidade, y, f"{servico.servico.valor:>8.3f}")
                    c.drawString(coluna_valor_unitario, y, f"{servico.desconto:>18.2f}")
                    c.drawString(coluna_total, y, f"{servico.profissional.nome:>18}")
