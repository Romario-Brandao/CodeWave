...
def folha_pagamento(request):
        ...
        if dados.get('acao') == 'Gravar':
            ...
            if Dias_uteis.objects.filter(mes_ano=mes_ano).exists():
                ...
                for chave, valor in dados.items():
                    if chave.startswith('funcionario'):
                        ...
                        obs = Decimal(dados.get(obs_key, 0))  # Recuperar o valor de 'dias_extras'
            ...
            for chave, valor in dados.items():
                if chave.startswith('funcionario'):
                    ...
                    obs_key = f'obs{valor}'  # Construir a chave
                    obs = Decimal(dados.get(obs_key, 0))  # Recuperar o valor de 'OBS'
    ...
    for item in funcionarios:
        ...
        folha =Folha_pagamento.objects.get(mes_ano_competencia=mes_ano, funcionario=item) or None         
        # relatorio.append({
        #     'funcionario': item,
        ...
        #     'pg_salario': folha.pg_salario if folha.pg_salario else '',
        ...
        relatorio.append({
            'funcionario': item,
            'inss': str(folha.inss if folha.inss else inss).replace('.', ','),
            'irrf': str(folha.irrf if folha.irrf else irrf).replace('.', ','),
            'desconto_vt': str(folha.desconto_vt if folha.desconto_vt else desconto_vt).replace('.', ','),
            'salario_liquido': str(folha.salario_liquido if folha.salario_liquido else salario_liquido).replace('.', ','),
            'pg_salario': folha.pg_salario if folha.pg_salario else '',
            'dias_extras': str(folha.dias_extras if folha.dias_extras else 0).replace('.', ','),
            'va': str(folha.va if folha.va else va).replace('.', ','),
            'vt': str(folha.vt if folha.vt else vt).replace('.', ','),
            'pg_va_vt': folha.pg_va_vt if folha.pg_va_vt else '',
            'fgts': str(folha.fgts if folha.fgts else fgts).replace('.', ','),
            'obs': folha.obs if folha.obs else '',
        })
    ...
    #         relatorio.append({
    #             'funcionario': item,
    #             'inss': inss if inss else inss,
    #             'irrf': irrf if irrf else irrf,
    #             'desconto_vt': desconto_vt if desconto_vt else desconto_vt,
    #             'salario_liquido': salario_liquido if salario_liquido else salario_liquido,
    #             'dias_extras': '0',
    #             'va': va if va else va,
    #             'vt': vt if vt else vt,
    #             'pg_va_vt': '',
    #             'fgts': fgts if fgts else fgts,
    #             'obs': '',
    #         })
